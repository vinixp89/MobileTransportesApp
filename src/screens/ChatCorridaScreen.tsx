...
const REMETENTE_CLIENTE = 0
const INTERVALO_MS = 3000

// Chat entre cliente e motorista enquanto a corrida está confirmada/em andamento — polling
// incremental (só busca mensagem nova, ver "desde"), mesmo endpoint usado pelo painel web
// (POST/GET /Corridas/{id}/mensagens).
  ...
  return (
    <KeyboardAvoidingView style={styles.tela} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={90}>
      <FlatList
        ...
        renderItem={({ item }) => {
          const minha = item.remetenteTipo === REMETENTE_CLIENTE
          return (
        ...
        ListEmptyComponent={<Text style={styles.vazio}>Nenhuma mensagem ainda — diga oi pro motorista.</Text>}
      />
